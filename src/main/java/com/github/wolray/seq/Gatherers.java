...
public final class Gatherers {
    ...
    public static <TR> Gatherer<TR, ?, List<TR>> windowFixed(int windowSize) {
        ...
        class FixedWindow {
            ...
            boolean integrate(TR element, Gatherer.Downstream<? super List<TR>> downstream) {
                ...
                if (at < windowSize) {
                    ...
                    return downstream.push(List.of(((TR) oldWindow)));
                }
            }
            ...
            void finish(Gatherer.Downstream<? super List<TR>> downstream) {
                if (at > 0 && !downstream.isRejecting()) {
                    ...
                    at = 0;
                    //noinspection unchecked
                    downstream.push(List.of((TR) lastWindow));
                }
            }
        }
        return Gatherer.<TR, FixedWindow, List<TR>>ofSequential(
                ...
                // Integrator
                Gatherer.Integrator.<FixedWindow, TR, List<TR>>ofGreedy(FixedWindow::integrate),
    ...
    public static <TR> Gatherer<TR, ?, List<TR>> windowSliding(int windowSize) {
        ...
        class SlidingWindow {
            ...
            boolean integrate(TR element, Gatherer.Downstream<? super List<TR>> downstream) {
                ...
                if (at < windowSize) {
                    ...
                    at -= 1;
                    firstWindow = false;
                    List<TR> element1 = new ArrayList<>();
                    for (int i = 0; i < windowSize; i++) {
                        element1.add((TR) oldWindow[i]);
                    }
                    return downstream.push(element1);
                }
            }
        ...
        return Gatherer.<TR, SlidingWindow, List<TR>>ofSequential(
                ...
                // Integrator
                Gatherer.Integrator.<SlidingWindow, TR, List<TR>>ofGreedy(SlidingWindow::integrate),
    ...
    public static <T, R> Gatherer<T, ?, R> scan(
        ...
        return Gatherer.ofSequential(State::new,
                Gatherer.Integrator.<State, T, R>ofGreedy(State::integrate));
    }
    ...
    enum Value implements Supplier, BinaryOperator, BiConsumer {
        ...
        <T, R> BiConsumer<T, Gatherer.Downstream<? super R>> finisher() {
            return (BiConsumer<T, Gatherer.Downstream<? super R>>) this;
        }
    }
    ...
    record GathererImpl<T, A, R>(
            ...
            @Override
            BiConsumer<A, Downstream<? super R>> finisher) implements Gatherer<T, A, R> {
